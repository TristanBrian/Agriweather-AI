...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = `hourly:${lat},${lon}:${days}:${units}`;
  const cached = getFromCache(cacheKey);
  if (cached) return NextResponse.json(cached);

  try {
    const data = await weatherAIGet('/v1/hourly', { lat, lon, days, ai: 'false', units });
    ...
    return NextResponse.json({ error: 'Could not load hourly forecast.' }, { status: 502 });
  }
}